...
async function fetchDashboardData() {
  ...
  const [
    ...
    db.select({ count: count() }).from(leads).where(and(eq(leads.clinicId, CLINIC_ID), eq(leads.temperature, "cold"))),
  ]);
  ...
  return (
    <div>
      ...
      <div className="kpi-strip">
        ...
        <div className="metric">
          <div className="metric-header">
            <span className="metric-icon"><Moon size={14} /></span>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              ...
              <tbody>
                {data.recentLeads.map((lead, i) => (
                  <tr key={lead.id} style={{ background: i % 2 === 1 ? "var(--surface-soft)" : "transparent", borderBottom: i < data.recentLeads.length - 1 ? "1px solid var(--line)" : "none" }}>
                    <td style={{ padding: "12px 16px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <div className="avatar" style={{ width: 34, height: 34, fontSize: 13, borderRadius: 9 }}>
                          {nameInitial(lead.name, lead.phone)}
                        </div>
                        <div>
                          <div style={{ fontWeight: 600, color: "var(--text)", lineHeight: 1.3 }}>{lead.name ?? "—"}</div>
                          <div style={{ fontSize: 12, color: "var(--muted)" }}>{lead.phone ?? "sem telefone"}</div>
                        </div>
                      </div>
                    </td>
                    <td style={{ padding: "12px 16px", color: "var(--text-soft)" }}>{channelLabel(lead.channel)}</td>
                    <td style={{ padding: "12px 16px" }}>
                      {lead.temperature ? (
                    ...
                    <td style={{ padding: "12px 16px" }}>
                      ...
                      <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 2 }}>{relativeTime(lead.createdAt)}</div>
                    </td>
                  </tr>
                ))}
              </tbody>
