...
export async function POST(req: NextRequest) {
  ...
  if (conversationIds.length > 0) {
    await db.delete(outboundMessages).where(inArray(outboundMessages.conversationId, conversationIds));
    await db.delete(messages).where(inArray(messages.conversationId, conversationIds));
    await db.delete(conversationStates).where(inArray(conversationStates.conversationId, conversationIds));
  }

  await db.delete(inboundEvents).where(eq(inboundEvents.clinicId, E2E_CLINIC_ID));
