 ...
 * personagens. A regressão foi descoberta lendo conversas, não pelo produto.
 */
describe("lintPersonaCoherence", () => {
  it("avisa quando o nome configurado não aparece na saudação (caso Horizonte)", () => {
    const w = lintPersonaCoherence(
      "Olá! Sou a especialista comercial com IA da Clínica Horizonte. Como posso ajudar?",
      "Marina",
    );
  ...
  it("não avisa quando a saudação nomeia a especialista comercial", () => {
    const w = lintPersonaCoherence(
  ...
  it("não avisa após o conserto da Horizonte (saudação passou a nomear a Marina)", () => {
    const w = lintPersonaCoherence(
      "Olá! Me chamo Marina, sou a especialista comercial com IA da Clínica Horizonte. Me conta o que você precisa: valores, agendamento ou tirar uma dúvida?",
      "Marina",
    );
    expect(w).toEqual([]);
  });
