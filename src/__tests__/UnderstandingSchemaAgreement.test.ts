...
function accepts(request: string, service: string | null): boolean {
  try {
    parseDentalUnderstanding({
      ...
      entities: {
        service,
        businessInformationTopic: request === "business-information" ? "address" : null,
        date: null,
        period: null,
        time: null,
        serviceCandidates: request === "compare-services" ? ["Clareamento", "Faceta"] : null,
        faqQuestion: request === "frequently-asked-question" ? "Preciso de encaminhamento?" : null,
        quantity: null,
        quantityScope: null,
        objectionQuestion: request === "registered-objection" ? "Está caro para mim" : null,
        ordinal: null,
      },
