...
const gateInput = { automationEnabled: true, duplicate: false, humanControlled: false, optedOut: false };

describe("pipeline operacional dental", () => {
  it("conflito de safety bloqueia reads e writes de todas as capabilities", async () => {
    const resolveService = vi.fn(); const bookSlot = vi.fn();
    const pack = createDentalPack({
      catalogRead: { resolveService },
      schedulingRead: { listSlots: vi.fn(), resolveOfferedSlot: vi.fn(), resolvePendingAppointment: vi.fn() },
      schedulingWrite: { bookSlot, confirmAppointment: vi.fn() },
    });
    const result = await runTurnPipeline({
      gateInput, state: { phase: "active", pendingStepId: null, completedStepIds: [] }, policy, now: new Date(0),
      understand: async () => ({ version: UNDERSTANDING_VERSION, request: "price-of-service", dialogueMove: "new_topic", entities: { service: "clareamento" }, signals: {}, safety: { requestsHuman: true }, confidence: 1, ambiguity: null }),
      capabilities: pack.capabilities, buildPlan: buildV2AuthorizedResponsePlan,
      compose: async () => ({ text: "unreachable", parts: [] }), validate: () => true,
    });
  ...
  it("dependency ausente bloqueia execute", async () => {
    ...
    const dependent: Capability<"work", Record<string, never>> = {
      id: "dependent",
      claim: () => ({ capabilityId: "dependent", confidence: 1, reason: "test", attributes: {}, dependsOn: ["missing"] }),
      decide: async () => ({ kind: "close" }),
      execute: async () => { writes += 1; return { type: "written", facts: [] }; },
    };
    const result = await runTurnPipeline({
      gateInput, state: { phase: "active", pendingStepId: null, completedStepIds: [] }, policy: {}, now: new Date(0),
      ...
      capabilities: [dependent], buildPlan: buildV2AuthorizedResponsePlan,
      compose: async () => ({ text: "unreachable", parts: [] }), validate: () => true,
    });
    expect(result.status).toBe("escalated");
