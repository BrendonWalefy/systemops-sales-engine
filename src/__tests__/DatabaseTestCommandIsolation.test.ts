...
const authoritySuite = [
  ...
  "src/__tests__/AiContractRejectionDatabase.test.ts",
  "src/__tests__/V2JourneyMediaDatabase.test.ts",
...
describe("database test command isolation", () => {
  it("runs calendar and embedded authority suites in separate processes", () => {
    ...
    expect(packageJson.scripts["test:db:authority"]).toBe(
      "vitest run src/__tests__/ScheduledBurstDebounceDatabase.test.ts src/__tests__/WhatsAppAuthorityBackfillDatabase.test.ts src/__tests__/WhatsAppTerminalLegacySettlementDatabase.test.ts src/__tests__/WhatsAppTerminalLegacyOutboundSettlementDatabase.test.ts src/__tests__/WhatsAppStreamPerformance.test.ts src/__tests__/SystemOpsDentalLabConfigDatabase.test.ts src/__tests__/V2OnlyRuntimeDatabase.test.ts src/__tests__/V2OnlyRolloutDatabase.test.ts src/__tests__/AiContractRejectionDatabase.test.ts src/__tests__/V2JourneyMediaDatabase.test.ts src/__tests__/helpers/runtime-performance-sql-recorder.test.ts --maxWorkers=1",
    );
    ...
    expect(packageJson.scripts.test).toContain(
      "--exclude src/__tests__/WhatsAppTerminalLegacyOutboundSettlementDatabase.test.ts",
    );
    expect(packageJson.scripts.test).toContain(
    ...
    expect(packageJson.scripts.test).toContain(
      "--exclude src/__tests__/AiContractRejectionDatabase.test.ts",
    );
    expect(packageJson.scripts.test).toContain(
      "--exclude src/__tests__/V2JourneyMediaDatabase.test.ts",
    );
    expect(packageJson.scripts.test).toContain(
