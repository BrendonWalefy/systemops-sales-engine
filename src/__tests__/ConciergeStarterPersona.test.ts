...
describe("buildConciergeStarter — persona humana", () => {
  const AURORA_OPENER =
    "Olá, tudo bem? Me chamo Gleice, sou da Clínica Aurora. Vi que você se interessou pelos nossos casos de lentes em resina. Me conta, você quer entender melhor como funciona, ver valores ou já procurar um horário para avaliação?";

  it("usa o opener curado da clínica e nunca diz 'assistente virtual'", () => {
    const clinic = makeClinic({ greetingMessage: AURORA_OPENER });
  ...
  it("usa greetingMessage verbatim quando ele já abre com saudação temporal", () => {
    const clinic = makeClinic({
      greetingMessage: "Boa tarde! Aqui é a Gleice da Aurora, como posso ajudar?",
    });
    const result = buildConciergeStarter(clinic, SAO_PAULO, "Alex", "Gleice");
    expect(result).toBe("Boa tarde! Aqui é a Gleice da Aurora, como posso ajudar?");
    expect(result).not.toMatch(/assistente virtual/i);
  });
  ...
  it("fallback sem greetingMessage: usa a recepcionista do editorial, sem 'assistente virtual'", () => {
    ...
    const result = buildConciergeStarter(clinic, SAO_PAULO, "Alex", "Gleice");
    expect(result).toContain("Sou a Gleice, da Clínica Aurora.");
    expect(result).not.toMatch(/assistente virtual/i);
  ...
  it("fallback sem greeting e sem recepcionista: fala como 'a equipe'", () => {
    ...
    expect(result).toContain("Aqui é a equipe da Clínica Aurora.");
    expect(result).not.toMatch(/assistente virtual/i);
  });
