...
const rating = { pairs: [{ run: 1, caseId: "price-0001", ratings: [{ factuallyCorrect: true, addressedWhatTheLeadRaised: true, advancedTheJourney: true, wouldRepeatToday: true }, { factuallyCorrect: true, addressedWhatTheLeadRaised: true, advancedTheJourney: true, wouldRepeatToday: true }] }] };
...
describe("Cycle I human review", () => {
  it("uses deterministic blind ordering and requires two complete reviewers", () => {
    ...
    expect(() => scoreHumanReview({ sheet, reviewerA: rating, reviewerB: { pairs: [] } })).toThrow(/missing/i);
  });

  it("retains reviewer-level scores, ties, and disagreements", () => {
    const sheet = buildBlindHumanReviewSheet({ runDigest: ref, pairs: [pair] });
    const score = scoreHumanReview({ sheet, reviewerA: rating, reviewerB: rating });
