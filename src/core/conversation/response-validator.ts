...
export function validateComposedResponse(input: {
  ...
  const text = input.response.text.trim();
  const captions = input.response.parts
    .filter((part): part is Extract<ComposedResponse["parts"][number], { type: "media" }> => part.type === "media")
    .map((part) => part.caption?.trim())
    .filter((caption): caption is string => Boolean(caption));
  const textToValidate = [text, ...captions.filter((caption) => caption !== text)].join("\n");

  if (!text && input.response.parts.length === 0) violations.add("empty_response");
  if (input.response.text.length > input.plan.maxCharacters) violations.add("response_too_long");
  if (countQuestions(textToValidate) > input.plan.maxQuestions) violations.add("too_many_questions");
...
function extractScheduleFacts(text: string): ScheduleFact[] {
  ...
  for (const match of text.matchAll(timePattern)) {
    const beforeTime = text.slice(Math.max(0, match.index! - 48), match.index);
...
function isSameScheduleFact(candidate: ScheduleFact, allowed: ScheduleFact): boolean {
  return candidate.time === allowed.time
...
function hasUnsupportedGuarantee(text: string): boolean {
  ...
  return /\b(?:resultado\s+(?:100\s*%\s*)?garantid[oa]|100\s*%\s*garantid[oa]|sem\s+(?:nenhum\s+)?risco|resultado\s+certo|(?:prometemos|prometo|garantimos|garanto)\s+(?:um\s+)?resultado|resultado\s+assegurado)\b/.test(normalized);
}
