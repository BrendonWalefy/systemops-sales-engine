...
import {
  ...
  verifyConfiguredCycleIReviewCalibrationAuthority,
  verifyConfiguredCycleIReviewRatingAuthority,
...
export { CYCLE_I_REVIEW_CALIBRATION_AUTHORITY_DOMAIN, CYCLE_I_REVIEW_RATING_AUTHORITY_DOMAIN };
...
export type ApprovedReviewerCalibrationManifest = Readonly<{ version: "conversation-v2-reviewer-calibration.v1"; rubricDigest: typeof APPROVED_REVIEWER_RUBRIC_DIGEST; manifestDigest: HmacRef; runManifestDigest: HmacRef; reviewers: readonly Readonly<{ reviewerRef: HmacRef; calibrationDigest: HmacRef; evidenceDigest: HmacRef; rates: Readonly<Record<keyof HumanReviewRating, number>> }>[]; authoritySignature: Ed25519SignatureRef }>;
...
const reviewerSchema = z.object({ reviewerRef: hmac, calibrationDigest: hmac, runManifestDigest: hmac, runDigest: hmac, sheetDigest: hmac, authoritySignature: signature, pairs: z.array(z.object({ run, caseId: id, ratings: z.tuple([ratingSchema, ratingSchema]) }).strict()) }).strict();
const calibrationSchema = z.object({ version: z.literal("conversation-v2-reviewer-calibration.v1"), rubricDigest: z.literal(APPROVED_REVIEWER_RUBRIC_DIGEST), manifestDigest: hmac, runManifestDigest: hmac, reviewers: z.array(z.object({ reviewerRef: hmac, calibrationDigest: hmac, evidenceDigest: hmac, rates: z.object({ factuallyCorrect: z.number().min(0.8).max(1), addressedWhatTheLeadRaised: z.number().min(0.8).max(1), advancedTheJourney: z.number().min(0.8).max(1), wouldRepeatToday: z.number().min(0.8).max(1) }).strict() }).strict()).min(2), authoritySignature: signature }).strict();
const dimensions = ["factuallyCorrect", "addressedWhatTheLeadRaised", "advancedTheJourney", "wouldRepeatToday"] as const;
const registeredScores = new WeakMap<object, Readonly<{ authority: AuthorizedCycleIRunManifest; runDigest: HmacRef }>>();
...
export function digestHumanReviewSheet(sheet: HumanReviewSheet): HmacRef { return `hmac:${createHash("sha256").update(`cycle-i-human-review-sheet.v1\0${JSON.stringify(canonical(sheet))}`).digest("hex")}`; }
function serializeUnsigned(input: unknown): string { const value = input as Record<string, unknown>; return JSON.stringify(canonical(Object.fromEntries(Object.entries(value).filter(([key]) => key !== "authoritySignature")))); }
export function serializeReviewerCalibrationAuthorityPayload(input: unknown): string { return serializeUnsigned(input); }
export function serializeReviewerRatingAuthorityPayload(input: unknown): string { return serializeUnsigned(input); }
function key(pair: { run: number; caseId: string }) { return `${pair.run}:${pair.caseId}`; }
export function parseApprovedReviewerCalibrationManifest(input: unknown, authority: AuthorizedCycleIRunManifest): ApprovedReviewerCalibrationManifest { if (!isRegisteredAuthorizedCycleIRunManifest(authority)) throw new Error("human review requires registered run authority"); const parsed = calibrationSchema.parse(input); if (parsed.runManifestDigest !== authority.manifestDigest) throw new Error("reviewer calibration is not bound to run manifest"); if (!verifyConfiguredCycleIReviewCalibrationAuthority(serializeReviewerCalibrationAuthorityPayload(parsed), parsed.authoritySignature as Ed25519SignatureRef)) throw new Error("reviewer calibration authority signature is invalid"); if (new Set(parsed.reviewers.map((item) => item.reviewerRef)).size !== parsed.reviewers.length) throw new Error("duplicate calibrated reviewer"); return freeze(parsed) as ApprovedReviewerCalibrationManifest; }
...
function scoreReviewer(input: unknown, expected: HumanReviewSheet, pairs: readonly ApprovedEvalPair[], calibration: ApprovedReviewerCalibrationManifest, runDigest: HmacRef, authority: AuthorizedCycleIRunManifest) {
  const reviewer = reviewerSchema.parse(input); if (reviewer.runManifestDigest !== authority.manifestDigest || reviewer.runDigest !== runDigest || reviewer.sheetDigest !== digestHumanReviewSheet(expected)) throw new Error("reviewer rating is not bound to the authorized run/sheet"); if (!verifyConfiguredCycleIReviewRatingAuthority(serializeReviewerRatingAuthorityPayload(reviewer), reviewer.authoritySignature as Ed25519SignatureRef)) throw new Error("reviewer rating authority signature is invalid"); if (reviewer.pairs.length !== expected.entries.length) throw new Error("missing human review rating"); const pairMap = new Map(pairs.map((pair) => [key(pair), pair])); const seen = new Set<string>();
...
export function scoreHumanReview(input: { sheet: unknown; pairs: readonly ApprovedEvalPair[]; runDigest: string; authority: AuthorizedCycleIRunManifest; calibrationManifest: unknown; reviewerA: unknown; reviewerB: unknown }): HumanReviewScore {
  if (!isRegisteredAuthorizedCycleIRunManifest(input.authority)) throw new Error("human review requires registered run authority");
  ...
  const runDigest = hmac.parse(input.runDigest) as HmacRef; const calibration = parseApprovedReviewerCalibrationManifest(input.calibrationManifest, input.authority); const a = scoreReviewer(input.reviewerA, expected, input.pairs, calibration, runDigest, input.authority), b = scoreReviewer(input.reviewerB, expected, input.pairs, calibration, runDigest, input.authority); if (a.reviewerRef === b.reviewerRef) throw new Error("human reviewers must be distinct");
  ...
  registeredScores.set(result, freeze({ authority: input.authority, runDigest }));
  return result;
}

export function isRegisteredHumanReviewScore(input: HumanReviewScore | null, authority: AuthorizedCycleIRunManifest, runDigest: HmacRef): input is HumanReviewScore {
  const binding = input === null ? undefined : registeredScores.get(input);
  return binding?.authority === authority && binding.runDigest === runDigest;
}
