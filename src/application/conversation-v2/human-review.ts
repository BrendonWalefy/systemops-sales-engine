...
export function buildBlindHumanReviewSheet(input: { runDigest: string; pairs: readonly ApprovedEvalPair[] }): HumanReviewSheet {
  ...
  const entries = input.pairs.map((pair) => { assertPair(pair); const pairKey = key(pair); if (seen.has(pairKey)) throw new Error(`duplicate human review pair: ${pairKey}`); seen.add(pairKey); const first = createHash("sha256").update(`${runDigest}:${pair.pairDigest}:${pairKey}`).digest()[0]! % 2 === 0 ? "v1" : "v2"; const byArm = { v1: pair.v1.outputText, v2: pair.v2.outputText }; const second = first === "v1" ? "v2" : "v1"; return freeze({ run: pair.run, caseId: pair.caseId, responses: [freeze({ position: 1 as const, outputText: byArm[first] }), freeze({ position: 2 as const, outputText: byArm[second] })] }); });
...
function scoreReviewer(input: unknown, expected: HumanReviewSheet, pairs: readonly ApprovedEvalPair[]) {
  ...
  const rows = reviewer.pairs.map((entry) => { const entryKey = key(entry); if (seen.has(entryKey)) throw new Error(`duplicate human review rating: ${entryKey}`); seen.add(entryKey); const pair = pairMap.get(entryKey); if (!pair) throw new Error(`unexpected human review rating: ${entryKey}`); const expectedEntry = expected.entries.find((item) => key(item) === entryKey); if (!expectedEntry) throw new Error(`missing human review pair: ${entryKey}`); const v1First = expectedEntry.responses[0].outputText === pair.v1.outputText && expectedEntry.responses[1].outputText === pair.v2.outputText; return freeze({ run: entry.run, caseId: entry.caseId, v1: entry.ratings[v1First ? 0 : 1], v2: entry.ratings[v1First ? 1 : 0] }); });
...
export function scoreHumanReview(input: { sheet: unknown; pairs: readonly ApprovedEvalPair[]; runDigest: string; reviewerA: unknown; reviewerB: unknown }): HumanReviewScore {
  ...
  const a = scoreReviewer(input.reviewerA, expected, input.pairs), b = scoreReviewer(input.reviewerB, expected, input.pairs); if (a.reviewerRef === b.reviewerRef) throw new Error("human reviewers must be distinct");
