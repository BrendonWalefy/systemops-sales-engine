...
const engineSummarySchema = z.object({
  status: z.enum(["observed", "unavailable", "unsupported", "error", "no_safe_response", "simulation_not_executed"]),
  ...
  finalTextCharacters: nullableNonNegativeInteger, finalTextDigest: hmacRef.nullable(), fallbackSource: z.enum(["draft", "repair", "fallback"]).nullable(), errorCode: z.enum(["provider_error", "shared_read_unavailable", "unknown_effect", "unsupported_request", "final_response_unavailable"]).nullable(), model: modelSchema.nullable(),
}).strict().superRefine((engine, context) => {
  if (engine.status !== "unavailable") return;
  const hasAttributedObservation = engine.understandingRequest !== null
    || engine.capabilityIds.length > 0
    || engine.decisionKinds.length > 0
    || engine.outcomeTypes.length > 0
    || engine.semanticClasses.length > 0
    || engine.finalTextCharacters !== null
    || engine.finalTextDigest !== null
    || engine.fallbackSource !== null
    || engine.model !== null;
  if (engine.errorCode !== "final_response_unavailable" || hasAttributedObservation) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      message: "unavailable engine cannot contain attributed observations",
    });
  }
});
...
const liveSchema = z.object({ version: z.literal(LIVE_COMPARISON_VERSION), turnRef: hmacRef, conversationRef: hmacRef.nullable(), inputRef: hmacRef, occurredAt: isoDateTime, commit, configDigest: hmacRef, datasetDigest: hmacRef.nullable(), v1: engineSummarySchema, v2: engineSummarySchema, comparisonStatus: z.enum(["comparable", "not_measurable"]), comparisonReason: z.literal("v1_final_response_unavailable").nullable(), intendedEffects: z.array(intendedEffectSchema), divergenceCodes: z.array(z.enum(["request_mismatch", "subject_mismatch", "outcome_mismatch", "critical_regression"])) }).strict().superRefine((record, context) => {
  if (record.comparisonStatus === "not_measurable") {
    if (
      record.comparisonReason === null
      || record.v1.status !== "unavailable"
      || record.divergenceCodes.length > 0
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message: "not measurable comparison requires an unavailable V1 arm and cannot claim divergence",
      });
    }
    return;
  }
  if (
    record.comparisonReason !== null
    || record.v1.status !== "observed"
    || record.v1.finalTextCharacters === null
    || record.v1.finalTextDigest === null
  ) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      message: "comparable record requires an observed final V1 response",
    });
  }
  if (record.v2.status === "unavailable") {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      message: "V2 does not use the V1 final-response-unavailable status",
    });
  }
});
