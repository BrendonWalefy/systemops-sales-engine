/**
 * Normalizador de texto para síntese de voz (TTS).
 *
 * Dono único da responsabilidade "tornar um texto falável": converte valores,
 * datas, horários e abreviações para a forma por extenso e remove formatação
 * visual (markdown, emojis, bullets) que o modelo leria literalmente.
 *
 * Não pertence a nenhum provider específico — é aplicado a TODOS via
 * NormalizingTtsGateway (decorator), para que cada gateway permaneça puro
 * (apenas texto → bytes de áudio).
 */
