/**
 * BW Odontologia — envia os dois vídeos de lentes a qualquer interesse no assunto.
 *
 * Altera só o campo `notes` da versão ativa:
 *   Passo 2 agora instrui o LLM a incluir os dois [MEDIA:id] (Simplificada + Estratificada)
 *   em vez de apenas "o vídeo mais relevante".
 *
 * Run: npx dotenv -e .env.local -- tsx scripts/update-bw-lentes-both-videos.ts
 */
...
Passo 1 — explique as duas técnicas com os valores em prosa corrida, sem listas: a Técnica Simplificada usa resina de alta qualidade e entrega um sorriso harmonioso e bonito, a partir de R$ 2.500 para vinte elementos. A Técnica Estratificada usa resina premium em múltiplas camadas, reproduzindo a translucidez, a profundidade e o brilho natural dos dentes, a partir de R$ 5.000. Em ambos os casos, o valor exato depende da avaliação presencial.

Passo 2 — apresente os dois vídeos com uma frase de introdução para cada um, depois inclua as tags de mídia na mesma ordem. Modelo obrigatório:
"Gravei dois vídeos mostrando cada técnica. O primeiro é da Técnica Simplificada, que mostra como fica o sorriso com a resina de alta qualidade em uma única camada. [MEDIA:3f5354c0-b6bc-460c-9dfe-e6df59517709] O segundo é da Técnica Estratificada, onde dá para ver a profundidade e o brilho que as múltiplas camadas de resina premium entregam. [MEDIA:70da6047-75bf-4062-8a20-6c7ef39a8682]"
Adapte o texto com naturalidade ao tom da conversa, mas mantenha a estrutura: frase descritiva → tag do vídeo → frase descritiva → tag do vídeo. NUNCA inverta a ordem. NUNCA omita um dos vídeos. NUNCA diga que vai "enviar depois" ou "avisar a equipe" — você mesmo envia com as tags acima.

Após os dois passos — ofereça o agendamento da avaliação: "O Dr. Gregorie avalia pessoalmente o seu sorriso e mostra exatamente como ficaria. A avaliação é R$ 100 e esse valor é abatido do tratamento se você avançar. Posso verificar um horário para você essa semana?"

Se o lead já indicou preferência por uma técnica (respondeu "estratificada", "simplificada", "a mais detalhada", etc.) e os vídeos ainda não foram enviados nesta conversa: valide a escolha com uma frase que aquece — algo como "A Estratificada é realmente o topo em personalização, o Dr. Gregorie faz um trabalho impressionante com ela" — e inclua os dois vídeos com a descrição de cada técnica — frase descritiva → [MEDIA:3f5354c0-b6bc-460c-9dfe-e6df59517709] → frase descritiva → [MEDIA:70da6047-75bf-4062-8a20-6c7ef39a8682] — para o lead ver o resultado antes de decidir. Em seguida ofereça o agendamento com a menção dos R$ 100.
...
async function main() {
  ...
  console.log("   Passo 2 agora envia os dois vídeos de lentes obrigatoriamente.");
  console.log("\n⚠️  Teste no simulador: /playbook/simulate → Produção → BW Odontologia");
  console.log('   A: "quero saber sobre lentes" → deve receber os 2 vídeos');
  console.log('   B: "tenho interesse em facetas" → mesmo resultado');
