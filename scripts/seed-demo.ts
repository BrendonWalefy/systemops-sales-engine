 ...
 * agenda são 100% calculados ao vivo, então este script gera VOLUME de dados
 * realista para que os números-herói do painel batam exatamente o kit de
 * marketing — sem nenhum dado real de pessoas.
 *
 * Idempotente: roda quantas vezes quiser. Cada execução APAGA e recria todos
 * os dados da clínica (reset limpo), datando os registros relativos ao momento
 * da execução — assim o painel fica "fresco" logo após semear.
 *
 * Uso:
 *   npm run seed:demo
 ...
 *
 * ── Números-alvo do painel (todos derivados de dados reais) ──
 *   Total de leads ............ 184
 *   Leads nos últimos 7 dias .. 42
 *   Consultas marcadas ........ 63   (conversão 63/184 = 34%)
 *   Consultas hoje ............ 6
 *   Autonomia IA .............. 97,8% (≈98%, com 4 conversas em atenção)
 *   Mensagens fora de horário . 58
 *   Tempo economizado ......... 42h  (~1.260 mensagens da IA)
 *   Leads quentes ativos ...... 12
 *   Receita potencial ......... R$ 68.400
 *   Receita confirmada ........ R$ 21.700
 *   ROI ....................... 480% (4,8x)
 *
 * ── Abas do Inbox ──
 *   Todas 184 · Quentes 12 · Mornos 21 · Frios 8 · Atenção 4 · Pausados 3 · Recuperação 9
 */
...
function addMsg(
  ...
  msgRows.push({ id: uuid(), conversationId, author, body, sentAt, intent: intent ?? null });
...
// ─────────────────────────────────────────────────────────────────────────
// Templates de conversa para figuração (curtas, realistas)
// ─────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────
// Insert em lotes (evita estourar limite de parâmetros)
// ─────────────────────────────────────────────────────────────────────────
...
async function main(): Promise<void> {
  ...
  const existing = await db
    ...
    .then((r) => r[0] ?? null);
  if (existing) {
    console.log("↻ Clínica demo existente encontrada — resetando dados…");
    await resetClinic(existing.id);
  }
  ...
  await db.insert(clinics).values({
    ...
    autoReplyEnabled: false, // segurança: nunca tenta responder via Z-API (clínica de demo)
    plan: PLAN,
    operationalStatus: "active",
    monthlyRevenueBrl: 452083, // calibra ROI = 21.700 / 4.520,83 = 4,8x (480%)
  ...
  // 2) profissionais
  const profHelena = uuid();
  const profRafael = uuid();
  ...
  // 3) membro admin (login da Dra. Helena → "Olá, Helena!" + visão financeira)
  ...
  // ───────────────────────────────────────────────────────────────────────
  // 6) LEADS / CONVERSAS / MENSAGENS / AGENDAMENTOS
  // ───────────────────────────────────────────────────────────────────────
  const activeConvIds: string[] = []; // hot/warm/cold — receberão msg final da IA + noturnas
  ...
  // controle do orçamento "últimos 7 dias" = 42 leads
  let recentBudget = 42;
  ...
  for (let i = 0; i < 12; i++) {
    ...
    const { convId } = makeLead({
      ...
      treatmentInterest: treatment, createdAt: created,
      channel: pick(CHANNELS, i), thread,
    });
    activeConvIds.push(convId);
    if (cast?.slots) {
      stateRows.push({
        id: uuid(), conversationId: convId, state: "slots_offered",
  ...
  for (let i = 0; i < 21; i++) {
    ...
    const { convId } = makeLead({
      ...
      temperature: "warm", treatmentInterest: treatment, createdAt: created,
      channel: pick(CHANNELS, i + 2), thread,
    });
    activeConvIds.push(convId);
  }
  ...
  // ── ATENÇÃO (4) — needsAttention (entra na conta da autonomia) ──
  ...
  // ── PAUSADOS (3) — aiPaused, sem needsAttention, takeover no futuro ──
  for (let i = 0; i < 3; i++) {
    const name = genName();
    makeLead({
      clinicId, name, status: "in_conversation", temperature: null,
      treatmentInterest: pick(["Lentes de porcelana", "Implante dentário", "Clareamento dental"], i),
      createdAt: createdAtFor(false), aiPaused: true, takeoverExpiresAt: spAt(-1, 18),
      channel: "whatsapp",
      thread: [
  ...
  // ── RECUPERAÇÃO (9) — follow_up_due, última msg da IA (não respondeu) ──
  for (let i = 0; i < 9; i++) {
    ...
    followRows.push({
      id: uuid(), clinicId, leadId, dueAt: spAt(-1, 9, i), status: "pending",
  ...
  // ── AGENDADOS (63) — status appointment_scheduled + appointment real ──
  // Receita potencial = soma dos valueCents (scheduled/confirmed) = R$ 68.400
  const valuePlan = buildValuePlan(63, 6_840_000);

  // Elenco da agenda da semana (seção 8) + extras premium.
  // (Patrícia/Felipe ficam em recuperação/atenção; aqui usamos nomes próprios.)
  const scheduledCast = [
    "Camila Rocha", "Mariana Alves", "Rafael Mendes", "Bruna Castro",
    "Larissa Monteiro", "Aline Barbosa",
  ];
  // 6 consultas HOJE (horários do dashboard "Consultas hoje")
  ...
  for (let i = 0; i < 63; i++) {
    ...
    const created = createdAtFor(i < 9); // 9 dos agendados contam nos "últimos 7 dias"
    const apptCreated = spAt(Math.floor(Math.random() * 6), 10, i); // sempre nos últimos 7d → entra no potencial

    let startsAt: Date;
    if (i < 6) startsAt = spAt(0, todayHours[i]); // hoje
    else startsAt = spAt(-(1 + Math.floor(Math.random() * 13)), 9 + (i % 9)); // próximos 14 dias
  ...
  // ── HISTÓRICO: GANHOS com consulta REALIZADA (14) — receita confirmada R$ 21.700 ──
  ...
  for (let i = 0; i < 14; i++) {
    ...
    const apptCreated = spAt(Math.floor(Math.random() * 6), 10, i + 20); // últimos 7d → entra no confirmado
  ...
  // ── HISTÓRICO restante para fechar 184 (20 ganhos sem valor + 30 perdidos) ──
  for (let i = 0; i < 20; i++) {
    makeLead({
  ...
  // ───────────────────────────────────────────────────────────────────────
  // 7) Conversas-herói (seção 6 do kit) — sobrescrevem o elenco
  //    Camila (Conv 1, agendada) e Lucas (Conv 3) já existem; reforçamos a
  //    Camila com o roteiro completo do Instagram → confirmação.
  // ───────────────────────────────────────────────────────────────────────
  // (Camila já está em scheduledCast[0]; seu thread genérico é suficiente para
  //  o vídeo. As linhas verbatim de marketing vivem no playbook/roteiro.)

  // ───────────────────────────────────────────────────────────────────────
  // 8) Mensagens fora do horário (58) — leads, à noite, nos últimos 7 dias
  // ───────────────────────────────────────────────────────────────────────
  const nightTexts = [
  ...
  // módulos do plano (habilita inbox, agenda, recuperação, etc.)
  ...
  console.log(`     conversas:    ${convRows.length}`);
  console.log(`     mensagens:    ${msgRows.length}  (IA: ${agentMsgCount} → ~${Math.round((agentMsgCount * 2) / 60)}h economizadas)`);
  console.log(`     agendamentos: ${apptRows.length}`);
  console.log(`     follow-ups:   ${followRows.length}`);
  console.log(`     msgs noturnas:${afterHoursCount}`);
